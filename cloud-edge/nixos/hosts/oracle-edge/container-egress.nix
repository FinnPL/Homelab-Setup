  ...
  networking.nftables.tables.container-egress = {
    ...
    content = ''
      chain forward {
        ...
        ip saddr 10.89.0.0/24 ip daddr 169.254.0.0/16 drop

        # Block container access to homelab/private networks.
        ip saddr 10.89.0.0/24 ip daddr 10.0.0.0/8 drop
        ip saddr 10.89.0.0/24 ip daddr 172.16.0.0/12 drop
        ip saddr 10.89.0.0/24 ip daddr 192.168.0.0/16 drop
        ip saddr 10.89.0.0/24 ip daddr 100.64.0.0/10 drop
      }
    '';
  };
}
